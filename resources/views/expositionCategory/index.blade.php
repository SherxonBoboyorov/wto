        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('admin.exposition-category.create') }}" class="btn btn-primary">Create</a>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @if (!empty($expositionCategories))
                                @foreach ($expositionCategories as $item)
                                    <tr>
                                        <td>{{ $item->id }}</td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.exposition-category.edit', ['exposition_category' => $item->id]) }}">
                                                    ...
                                                    <a type="button" onclick="setDeleteItem({{ $item->id }})"
                                                        ...
                                                        <form id="delete{{ $item->id }}"
                                                            action="{{ route('admin.exposition-category.destroy', ['exposition_category' => $item->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                ...
                {{ $expositionCategories->links('vendor.pagination.vuexy-admin') }}
            </div>
        </div>
    </div>
    <!-- Bordered table end -->
