...
@section('title', 'Museum collection')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('admin.museum-collection.create') }}" class="btn btn-primary">Create</a>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead>
                            ...
                            <form id="indexFilter" action="{{ route('admin.museum-collection.index') }}">
                                <tr id="filters" role="row">
                                    <th rowspan="1" colspan="1"><input type="text"  value="{{ request('id') }}" name="id"  class="form-control form-control-sm"
                                            placeholder="Id"></th>
                        ...
                        <tbody>
                            @if (!empty($museumCollections))
                                @foreach ($museumCollections as $item)
                                    <tr>
                                        <td>{{ $item->id }}</td>
                                        <td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.museum-collection.edit', ['museum_collection' => $item->id]) }}">
                                                    ...
                                                    <a type="button" onclick="setDeleteItem({{ $item->id }})"
                                                        ...
                                                        <form id="delete{{ $item->id }}"
                                                            action="{{ route('admin.museum-collection.destroy', ['museum_collection' => $item->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                ...
                {{ $museumCollections->links('vendor.pagination.vuexy-admin') }}
            </div>
        </div>
    </div>
    <!-- Bordered table end -->
