        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('admin.scientific-research.create') }}" class="btn btn-primary">Create</a>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead>
                            ...
                            <form id="indexFilter" action="{{ route('admin.scientific-research.index') }}">
                        ...
                        <tbody>
                            @if (!empty($scientificResearches))
                                @foreach ($scientificResearches as $item)
                                    <tr>
                                        <td>{{ $item->id }}</td>
                                        <td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.scientific-research.edit', ['scientific_research' => $item->id]) }}">
                                                    ...
                                                    <a type="button" onclick="setDeleteItem({{ $item->id }})"
                                                        ...
                                                        <form id="delete{{ $item->id }}"
                                                            action="{{ route('admin.scientific-research.destroy', ['scientific_research' => $item->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                ...
                {{ $scientificResearches->links('vendor.pagination.vuexy-admin') }}
            </div>
        </div>
    </div>
    <!-- Bordered table end -->
