        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('admin.leaders.create') }}" class="btn btn-primary">Create</a>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead>
                            ...
                            <form id="indexFilter" action="{{ route('admin.leaders.index') }}">
                            <tr id="filters" role="row">
                                ...
                                <th rowspan="1" colspan="1"><input type="text" name="name_uz"  value="{{ request('name_uz') }}" class="form-control form-control-sm"
                                        placeholder="Name Uz"></th>
                                <th rowspan="1" colspan="1"><input type="text" name="name_en"  value="{{ request('name_en') }}" class="form-control form-control-sm"
                                        placeholder="Name En"></th>
                                <th rowspan="1" colspan="1"></th>
                                <th rowspan="1" colspan="1"></th>
                            </tr>
                            </form>
                            ...
                            @if (!empty($leaders))
                                @foreach ($leaders as $item)
                                    <tr>
                                        <td>{{ $item->id }}</td>
                                        <td>
                                            <img src="{{ asset($item->image_url) }}" class="me-75" height="20"
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.leaders.edit', ['leader' => $item->id]) }}">
                                                    ...
                                                    <a type="button" onclick="setDeleteItem({{ $item->id }})"
                                                        ...
                                                        <form id="delete{{ $item->id }}"
                                                            action="{{ route('admin.leaders.destroy', ['leader' => $item->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                ...
                {{ $leaders->links('vendor.pagination.vuexy-admin') }}
            </div>
        </div>
    </div>
    <!-- Bordered table end -->
