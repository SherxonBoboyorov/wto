        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('admin.for-children.create') }}" class="btn btn-primary">Create</a>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead>
                            ...
                            <form id="indexFilter" action="{{ route('admin.for-children.index') }}">
                        ...
                        <tbody>
                            @if (!empty($forChildren))
                                @foreach ($forChildren as $item)
                                    <tr>
                                        <td>{{ $item->id }}</td>
                                        <td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.for-children.edit', ['for_child' => $item->id]) }}">
                                                    ...
                                                    <a type="button" onclick="setDeleteItem({{ $item->id }})"
                                                        ...
                                                        <form id="delete{{ $item->id }}"
                                                            action="{{ route('admin.for-children.destroy', ['for_child' => $item->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                ...
                {{ $forChildren->links('vendor.pagination.vuexy-admin') }}
            </div>
        </div>
    </div>
    <!-- Bordered table end -->
